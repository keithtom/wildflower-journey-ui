...
const RequestAdviceModal = ({ open, toggle, requestIsValid, requestAgain }) => {
  ...
  return (
    <Modal
      open={open}
      toggle={toggle}
      title={(hasContext && hasProposal && hasStakeholders) ? "Request advice" : requestAgain ? 'Request advice again' : "Oops, you can't request advice yet"}
    >
      <RequestAdviceContent toggle={toggle} hasContext={hasContext} hasProposal={hasProposal} hasStakeholders={hasStakeholders} requestAgain={requestAgain} />
    </Modal>
  )
}
