    ...
    if (token && !pagesWithNoUserRequired.includes(router.asPath)) {
      ...
      usersApi
        ...
        .then((result) => {
          ...
          // console.log(includedPersonBasic);
          ...
          if (!!userAttributes && !ignore) {
            ...
            //TODO: Resolve this temporary gate for the user test
            if (process.env === "production") {
              setIsOperationsGuide(
                user?.attributes?.email === "maya.warsame@wildflowerschools.org"
              );
            } else {
              setIsOperationsGuide(opsGuide);
            }
          }
